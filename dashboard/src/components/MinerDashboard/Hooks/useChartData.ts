...
export function useChartData(timeRange: string) {
  ...
  useEffect(() => {
    ...
    const selectedRange = TIME_RANGES.find((range) => range.value === timeRange) || TIME_RANGES[1] // Default to month
    ...
    while (currentDate <= endDate) {
      ...
      if (format === "day") {
        ...
        const weekNumber = Math.ceil(
          (currentDate.getDate() - 1 + new Date(currentDate.getFullYear(), currentDate.getMonth(), 1).getDay()) / 7,
        )
        label = `${currentDate.toLocaleDateString("en-US", { month: "short", day: "numeric" })}`
